...
def lighting():
 ...
 sc.world.use_nodes=True;sc.world.node_tree.nodes.get('Background').inputs[0].default_value=(.65,.75,.82,1);sc.world.node_tree.nodes.get('Background').inputs[1].default_value=.35
 bpy.ops.object.light_add(type='SUN',location=(0,0,80));sun=bpy.context.object;sun.name='Warm Bahrain afternoon sun';sun.data.energy=2.8;sun.data.angle=.15;sun.rotation_euler=(math.radians(46),math.radians(-28),math.radians(-28))
 ...
 sc.view_settings.view_transform='AgX';sc.view_settings.look='AgX - Medium High Contrast';sc.render.resolution_x=3840;sc.render.resolution_y=2160
...
def camera_for(v):
 ...
 return camera
VIEWS=[{'id':'01-overview','name':'Full event overview','position':[25,340,300],'target':[0,0,-6],'ortho':610},{'id':'02-waterfront','name':'Waterfront pavilions','position':[98,55,24],'target':[30,0,-55],'lens':32},{'id':'03-arrival','name':'Arrival and Owners Enclosure','position':[-172,62,-5],'target':[-125,1,-49],'lens':35},{'id':'04-concours','name':'Concours lawn and hospitality','position':[36,67,143],'target':[39,1,35],'lens':32},{'id':'05-lounges','name':'Hospitality lounges','position':[125,20,62],'target':[83,1,22],'lens':35},{'id':'06-interior','name':'Owners Enclosure furnished interior','position':[-105.7,1.9,-53.0],'target':[-105.7,1.5,-73],'lens':24}]
def scene_notes(scene):
 ...
 for filename in ['site-seed.json','calibration-report.json','furnishing-report.json']:
