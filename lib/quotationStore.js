...
function isMissingSupabaseObjectError(error) {
    ...
    return (
        ...
        code === '42883' ||
        code === 'PGRST202' ||
        message.includes('quotation_meta') ||
        message.includes('reserve_qt_number') ||
...
async function getHighestQuotationNumber() {
    if (SUPABASE_ENABLED) {
        ...
        return normalizeNumber(data?.[0]?.qt_number, 0);
    }

    const quotations = await readJson(QUOTATIONS_FILE, []);
    return quotations.reduce((max, quotation) => Math.max(max, normalizeNumber(quotation?.qt_number, 0)), 0);
}

async function getFallbackNextQuotationNumber() {
    const highestNumber = await getHighestQuotationNumber();
    return Math.max(DEFAULT_NEXT_NUMBER, highestNumber + 1);
}
...
function validateQuotationPayload(payload = {}, { isUpdate = false } = {}) {
    ...
    const clientOrg = normalizeText(payload.client_org);
    ...
    if (!isUpdate || projectTitle || createdBy || sections.length > 0) {
        ...
        if (!clientOrg) errors.push('Client company/name is required.');
    ...
    if (SUPABASE_ENABLED) {
        ...
        if (error) {
            if (isMissingSupabaseObjectError(error)) {
                return getFallbackNextQuotationNumber();
            }
            throw error;
        }
...
export async function setNextQuotationNumber(value) {
    ...
    if (SUPABASE_ENABLED) {
        ...
        if (error) {
            if (isMissingSupabaseObjectError(error)) {
                throw new QuotationStoreError('Quotation numbering metadata is not configured in Supabase yet. Run the backend restore/setup for quotation_meta and reserve_qt_number.', 500);
            }
            throw error;
        }
        return num;
