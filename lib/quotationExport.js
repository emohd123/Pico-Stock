...
function drawPageHeader(doc, quotation) {
    ...
    try {
        doc.image(logoFull, ML, 36, { width: 115 });
    } catch {
    ...
    doc.font('Helvetica-Bold').fontSize(9).fillColor(DARK)
       .text(profile.legalName, 305, ry, { width: 250, align: 'right' });
    ry += 14;
    ...
    doc.font('Helvetica-Bold').fontSize(8).fillColor(MEDIUM)
       .text(profile.vatNumber, 305, ry, { width: 250, align: 'right' });
...
function drawInfoBlock(doc, quotation, startY) {
    ...
    // TRN row (only shown if filled)
    const trn = String(quotation.client_trn || '').trim();
    ...
    if (trn) {
        doc.font('Helvetica').fontSize(7.5).fillColor(GRAY).text('TRN', ML, y);
        doc.font('Helvetica-Bold').fontSize(9.5).fillColor(DARK).text(trn, ML, y + 10, { width: 255 });
        y += 30;
    }
...
function drawSignatureFooter(doc, quotation, staffSig, y) {
    ...
    if (y + 90 > SAFE_BTM) { doc.addPage(); drawPageHeader(doc, quotation); y = 145; }

    hLine(doc, y, BORDER, 0.5);
    y += 12;

    doc.font('Helvetica-Bold').fontSize(9.5).fillColor(DARK_TEAL).text('Authorised Signatory', ML, y);
    y += 16;

    if (hasSig) {
        try {
            const sigBuf = Buffer.from(staffSig.signature_image.split(',')[1] || staffSig.signature_image, 'base64');
            doc.image(sigBuf, ML, y, { height: 52, fit: [130, 52] });
        } catch {}
    }
    if (hasStamp) {
        try {
            const stampBuf = Buffer.from(staffSig.stamp_image.split(',')[1] || staffSig.stamp_image, 'base64');
            doc.image(stampBuf, ML + 150, y, { height: 52, fit: [70, 70] });
        } catch {}
    }
    y += 60;

    doc.font('Helvetica').fontSize(8).fillColor(MEDIUM)
       .text(quotation.created_by || '', ML, y)
...
export async function generateQuotationPdf(quotation, mode = 'customer', staffSig = null) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // ── Scope heading ──────────────────────────────────────────────────────
        if (y + 20 > SAFE_BTM) { doc.addPage(); y = 40; drawPageHeader(doc, quotation); y = 135; }
        ...
        (quotation.sections || []).forEach((section, si) => {
            ...
            // Section header
            if (y + 18 > SAFE_BTM) { doc.addPage(); y = 40; drawPageHeader(doc, quotation); y = 135; y = drawTableHeader(doc, cols, y); }
            ...
            if (management) {
                if (y + 16 > SAFE_BTM) { doc.addPage(); y = 40; drawPageHeader(doc, quotation); y = 135; }
        ...
        // ── Commercial summary ─────────────────────────────────────────────────
        if (y + 100 > SAFE_BTM) { doc.addPage(); y = 40; drawPageHeader(doc, quotation); y = 135; }
        y = drawCommercialSummary(doc, quotation, y, management);

        // ── Internal notes (management only) ──────────────────────────────────
        if (management && quotation.notes?.trim()) {
            if (y + 40 > SAFE_BTM) { doc.addPage(); y = 40; drawPageHeader(doc, quotation); y = 135; }
