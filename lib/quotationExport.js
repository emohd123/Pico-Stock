...
function drawTitleBar(doc, quotation, mode = 'customer', headerMetrics = { headerBottom: 128, logoBottom: 88 }) {
    ...
    const labels = quotation.document_labels || {};
    drawPdfText(doc, pdfText(quotation.document_title) || 'QUOTATION', ML, titleY, { bold: true, size: 15, color: DARK_TEAL, forceLatin: true });
    if (mode === 'management') {
    ...
    if (quotation.ref) {
        drawPdfText(doc, `${labels.ref || 'PO Number'}:  ${quotation.ref}`, ML, dateY + 15, { size: 9, color: MEDIUM });
    }
...
function drawInfoBlock(doc, quotation, startY) {
    ...
    const quoteNo = pdfText(quotation.document_number) || (quotation.qt_number ? `QT-${quotation.qt_number}` : '—');
    const labels = quotation.document_labels || {};
    ...
    let ly = startY + 9;
    drawPdfText(doc, labels.billTo || 'BILL TO', leftX + 10, ly, { size: 7.2, color: GRAY, forceLatin: true });
    ly += 12;
    ...
    const metaRows = [
        [labels.date || 'Quote Date', quoteDate],
        [labels.preparedBy || 'Prepared By', createdBy],
        [labels.number || 'Quote No.', quoteNo],
    ];
    let ry = startY + 10;
...
export async function generateQuotationPdf(quotation, mode = 'customer', staffSig = null) {
    ...
    return new Promise((resolve, reject) => {
        ...
        drawPdfText(doc, quotation.document_labels?.scope || 'SCOPE OF WORKS', ML, y, { bold: true, size: 10.5, color: DARK_TEAL, forceLatin: true });
        y += 14;
        ...
        const extraBlocks = (Array.isArray(quotation.extra_blocks) ? quotation.extra_blocks : [])
            .filter((block) => block && block.title && Array.isArray(block.items))
            .map((block) => [String(block.title), block.items]);
        const hasLegalBlocks = [
            ...extraBlocks.map(([, items]) => items),
