...
export async function updateQuoteNotesAction(formData) {
    ...
    // Status is no longer edited here, so carry the stored one through rather
    // than letting a missing field silently reset it.
    const existing = await getQuotationForMinistry(ministryId, quoteId);
    await updateQuoteNotes(quoteId, String(formData.get('notes') || '').trim(), (existing && existing.status) || 'submitted');
    // The quotation number, editable because meetings are often filed under one
    // number and split later. Only the tail changes; the Q/MM/YYYY/EM prefix is
    // rebuilt from the reference already on the row.
    const wantedNo = String(formData.get('quoteNo') || '').trim();
    if (existing && /^\d{3,8}$/.test(wantedNo)) {
        // One early row carries a bare number with no prefix; there the whole
        // reference is the number, so the head is simply empty.
        const cut = existing.ref.lastIndexOf('/');
        const head = cut >= 0 ? existing.ref.slice(0, cut + 1) : '';
        const nextRef = `${head}${wantedNo}`;
        if (nextRef !== existing.ref) {
            await setQuotationRef(quoteId, nextRef);
            await logActivity({
                ministryId, quotationId: quoteId, actor: 'admin', action: 'quotation.renumbered',
                detail: `Quotation renumbered ${existing.ref} → ${nextRef}`,
            });
        }
    }
    const kind = String(formData.get('meetingKind') || '') === 'side' ? 'side' : 'main';
    if (existing && existing.meetingKind !== kind) {
        await setQuotationMeeting(quoteId, { kind });
        await logActivity({
            ministryId, quotationId: quoteId, actor: 'admin', action: 'quotation.updated',
            detail: `${existing.ref} marked as a ${kind === 'side' ? 'SIDE' : 'MAIN'} meeting`
                + (kind === 'side' ? ' — planned as its own room, never sharing a build with the main meeting' : ''),
        });
    }
    revalidatePath(`/quotations/ministry/${ministryId}`);
    revalidatePath('/quotations');
