...
export async function setMinistryNoteFile(ministryId, noteId, file) {
    // Removing the file leaves `show_to_production` alone: the note's own text
    // is worth sharing on its own, and the tick is the admin's decision, not a
    // side effect of swapping an attachment.
    const update = file
        ? { file_url: file.url, file_name: file.name, file_type: file.type || null, file_size_bytes: file.size || null }
        : { file_url: null, file_name: null, file_type: null, file_size_bytes: null };
...
// The notes production is allowed to see. A note counts whether or not it
// carries a file — "kit stays overnight, guard arranged" is worth passing on
// with nothing attached.
export async function getSharedNotes(ministryId) {
    const { data, error } = await getClient().from(T.notes).select('*')
        .eq('ministry_id', ministryId).eq('show_to_production', true)
...
// The LPO documents themselves, stored alongside the tick so they can be read
// back later — "was it actually received" is a question the checkbox alone
// answers with nothing to show. A ministry can hold several: one per meeting,
// an amended one, the signed copy that followed the emailed one.
// quotationId names the single meeting this purchase order pays for. Null means
// it covers the ministry as a whole, which is what every LPO meant before the
// column existed.
function mapLpo(r) {
    return { id: r.id, ministryId: r.ministry_id, blobUrl: r.blob_url, fileName: r.file_name,
        fileType: r.file_type, sizeBytes: r.size_bytes, uploadedAt: r.uploaded_at,
        quotationId: r.quotation_id ?? null };
}
export async function getMinistryLpos(ministryId) {
    const { data, error } = await getClient().from(T.lpos).select('*')
        .eq('ministry_id', ministryId).order('uploaded_at', { ascending: false });
    fail(error);
    return (data || []).map(mapLpo);
}
export async function getMinistryLposBulk(ministryIds) {
    const map = new Map();
    if (!ministryIds.length) return map;
    const { data, error } = await getClient().from(T.lpos).select('*')
        .in('ministry_id', ministryIds).order('uploaded_at', { ascending: false });
    fail(error);
    for (const r of data || []) {
        if (!map.has(r.ministry_id)) map.set(r.ministry_id, []);
        map.get(r.ministry_id).push(mapLpo(r));
    }
    return map;
}
export async function getMinistryLpo(id) {
    const { data, error } = await getClient().from(T.lpos).select('*').eq('id', id).limit(1);
    fail(error);
    return data && data.length ? mapLpo(data[0]) : null;
}
export async function addMinistryLpo(ministryId, file, quotationId = null) {
    const { error } = await getClient().from(T.lpos).insert({
        ministry_id: ministryId, blob_url: file.url, file_name: file.name,
        file_type: file.type || null, size_bytes: file.size || null,
        quotation_id: quotationId || null,
    });
    fail(error);
}
/** Point an LPO at one meeting, or back at the ministry as a whole (null). */
export async function setLpoQuotation(id, quotationId) {
    const { error } = await getClient().from(T.lpos)
        .update({ quotation_id: quotationId || null }).eq('id', id);
    fail(error);
}
export async function deleteMinistryLpo(id) {
    const { error } = await getClient().from(T.lpos).delete().eq('id', id);
    fail(error);
}
...
// Correct a quotation's reference. Several meetings often get filed under one
// number before each is issued its own, so this has to be editable after the
// fact — the number on the issued PDF is the one that must win.
export async function setQuotationRef(quotationId, ref) {
    const { error } = await getClient().from(T.quotations).update({ ref }).eq('id', quotationId);
    fail(error);
}
