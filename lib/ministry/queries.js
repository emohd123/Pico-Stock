...
export async function getProductionAssignments(quotationIds) {
    ...
    fail(error);
    for (const r of data || []) {
        map.set(`${r.quotation_id}:${r.item_no}`, { dept: r.dept, title: r.title || '', nameTags: r.name_tags || [] });
    }
    return map;
}
...
// Likewise column-scoped: the name-plate list for an item (see NAME_TAG_ITEM_NO).
export async function setProductionNameTags(quotationId, itemNo, tags) {
    const { error } = await getClient().from(T.production)
        .upsert({ quotation_id: quotationId, item_no: itemNo, name_tags: tags && tags.length ? tags : null }, { onConflict: 'quotation_id,item_no' });
    fail(error);
}
