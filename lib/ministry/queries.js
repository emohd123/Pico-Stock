...
export async function createMinistry({ name, nameAr, contactEmail }) {
    const { data, error } = await getClient().from(T.ministries)
        .insert({ name, name_ar: nameAr || null, contact_email: contactEmail || null, token: generateToken() })
        .select('*').single();
    fail(error);
    return mapMinistry(data);
}
export async function updateMinistryDetails(id, d) {
    const { error } = await getClient().from(T.ministries).update({
        ...
        po_box: d.poBox || null, contact_email: d.contactEmail || null, internal_note: d.internalNote || null,
    }).eq('id', id);
    fail(error);
}
export async function regenerateToken(id) {
    const { error } = await getClient().from(T.ministries).update({ token: generateToken() }).eq('id', id);
    fail(error);
}
