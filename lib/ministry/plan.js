...
import { SINGLE_STOCK_ITEM_NOS, SINGLE_STOCK_LABELS, isConsumable, deriveSchedule, isoAddDays, daysBetween } from './production';
...
export function computeInventory(meetings, namesByNo) {
    ...
    const itemNos = [...new Set(list.flatMap((m) => Object.keys(m.qtyByItem || {}).map(Number)))].sort((a, b) => a - b);
    const out = [];
    for (const no of itemNos) {
        ...
        if (isConsumable(no) || isCustomItemNo(no)) {
            const total = users.reduce((s, m) => s + m.qtyByItem[no], 0);
            out.push({
                no, name: namesByNo.get(no) || `Item ${no}`, kind: 'consumable',
                needed: total, meetings: users.length,
            });
        } else {
            // demand per calendar day = sum over meetings live that day
            const byDay = new Map();
            for (const m of users) for (const d of m.eventDays) byDay.set(d, (byDay.get(d) || 0) + m.qtyByItem[no]);
            let peak = 0, peakDay = null;
            for (const [d, n] of [...byDay].sort()) if (n > peak) { peak = n; peakDay = d; }
            const peakUsers = users.filter((m) => m.eventDays.includes(peakDay))
                .map((m) => ({ ministry: m.ministry, qty: m.qtyByItem[no] }));
            out.push({
                no, name: namesByNo.get(no) || `Item ${no}`, kind: 'reusable',
                needed: peak, peakDay, peakUsers, meetings: users.length,
