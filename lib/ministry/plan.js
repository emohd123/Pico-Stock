 ...
 */
/**
 * Where a meeting physically is, for the purpose of "can these two share a
 * build". A main meeting is identified by its venue. A side meeting runs in its
 * own room at the same time as the main one, so it is its own place even when
 * the hotel matches — treating it as the same venue would quietly hand one set
 * of furniture to two rooms at once.
 */
function placeOf(m) {
    if (m.side) return `side:${m.key}`;
    return m.venue;
}
...
export function buildPlan(meetings) {
    ...
    for (const itemNo of SINGLE_STOCK_ITEM_NOS) {
        ...
        for (const [iso, day] of days) {
            ...
            const known = [...new Set(needing.filter((m) => m.venue !== VENUE_UNKNOWN && !m.side).map((m) => placeOf(m)))];
            const unknown = needing.filter((m) => m.venue === VENUE_UNKNOWN || m.side);
            ...
            clashDays.push({
                ...
                // it at the other meeting's hotel dissolves the clash. A side
                // meeting does not — it is a second room by definition.
                resolvable: needing.some((m) => m.venue === VENUE_UNKNOWN && !m.side),
            });
            if (n > peak) {
                peak = n; peakDay = iso;
                peakWhere = [...known, ...unknown.map((m) => (m.side ? `${m.venue} (side)` : VENUE_UNKNOWN))];
            }
        }
