...
/**
 * One row per quotation number — the revision that supersedes the rest.
 *
 * A quotation is re-uploaded as it is revised, and every revision stays in the
 * table so the history survives. The production surfaces group a meeting by its
 * event date, which sweeps those old revisions back in and lists the same items
 * several times over. Production reading that sheet would build the job twice.
 *
 * Highest revision wins; on a tie the newer row does, which is what a straight
 * re-upload of the same revision produces.
 */
export function currentRevisionsOnly(quotes) {
    const best = new Map();
    for (const q of quotes) {
        const prev = best.get(q.ref);
        const newer = !prev
            || (q.revision || 0) > (prev.revision || 0)
            || ((q.revision || 0) === (prev.revision || 0) && q.id > prev.id);
        if (newer) best.set(q.ref, q);
    }
    return [...best.values()];
}
