...
/**
 * Readable size label from a Pico product name, for client-facing pages.
 * e.g. "... Bean Bag Black H70*D80cm" → "H70 × D80 cm"
 *      "... Meeting Table White H72*D75*W125cm" → "H72 × D75 × W125 cm"
 */
export function extractSizeLabel(name) {
    if (!name) return '';
    const decoded = decodeHtml(name);
    // "cm" is sometimes missing (e.g. "H73*D60*W140"), so accept either the unit
    // or at least two measurements.
    const match = decoded.match(/\b(H\s?\d+(?:\.\d+)?(?:\s*[*x×]\s*[A-Z]?\s?\d+(?:\.\d+)?)*)\s*(cm\b)?/i);
    if (!match || (!match[2] && !/[*x×]/i.test(match[1]))) return '';
    const parts = match[1].split(/\s*[*x×]\s*/i).map((part) => part.replace(/\s+/g, '').toUpperCase());
