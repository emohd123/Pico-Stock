...
export function tentFootprint(object) {
  ...
  if (object.roofType !== 'hexagon') return [[-width/2,-depth/2],[width/2,-depth/2],[width/2,depth/2],[-width/2,depth/2]];
...
export class EventStudioEngine {
  constructor(container, callbacks = {}) {
    ...
    this.world = new THREE.Scene(); this.world.background = new THREE.Color('#dce8e7');
    this.world.fog = new THREE.Fog('#dce8e7', 650, 1300);
    ...
    this.ambient = new THREE.HemisphereLight('#edf5ff', '#72775b', 2.1); this.world.add(this.ambient);
    this.sun = new THREE.DirectionalLight('#fff1d3', 3.2);
  ...
  mat(color = '#e7e2d4', extras = {}) {
    ...
    if (!this.materials.has(key)) this.materials.set(key, new THREE.MeshStandardMaterial({ color, roughness: .72, ...extras }));
  ...
  batchObject(root) {
    ...
    for(const meshes of groups.values()){
      if(meshes.length<2)continue;const geometries=meshes.map(mesh=>{mesh.updateMatrix();let geometry=mesh.geometry.clone().applyMatrix4(mesh.matrix);if(geometry.index){const expanded=geometry.toNonIndexed();geometry.dispose();geometry=expanded;}return geometry;});let merged;
  ...
  shape(points, material, parent, y = 0) {
  ...
  tent(o, root) {
    const [w,h,d] = o.dimensions, eave = Math.min(h-.2, Number(o.metadata?.eaveHeight) || Math.min(3.2,h*.62));
    ...
    if (o.roofType === 'hexagon') {
      const { points, segments } = tentWallSegments(o);
      const floor = this.shape(points, this.mat('#b6a78d'), root, .14); floor.userData.part = 'floor';
      const vertices=points.flatMap(([x,z])=>[x,eave,z]), indices=[];
      vertices.push(0,h,0);for(let i=0;i<points.length;i++)indices.push(i,points.length,(i+1)%points.length);
      const roofGeometry=new THREE.BufferGeometry();roofGeometry.setAttribute('position',new THREE.Float32BufferAttribute(vertices,3));roofGeometry.setIndex(indices);roofGeometry.computeVertexNormals();
      const roof=this.mesh(roofGeometry,ivory,root);roof.userData.part='roof';
      for(const [x,z] of points){const post=this.boxMesh(.13,eave,.13,frame,root,[x,eave/2,z]);post.userData.part='frame';}
      const sideMaterial=o.metadata?.wallStyle==='solid'?ivory:this.mat('#aac2c1',{transparent:true,opacity:.23,metalness:.1,roughness:.18,depthWrite:false,side:THREE.DoubleSide});
      for(const [a,b] of segments){const dx=b[0]-a[0],dz=b[1]-a[1],wall=this.boxMesh(Math.hypot(dx,dz),eave-.2,.07,sideMaterial,root,[(a[0]+b[0])/2,eave/2,(a[1]+b[1])/2]);wall.rotation.y=-Math.atan2(dz,dx);wall.userData.part='wall';}
      return;
    }
    this.boxMesh(w,.14,d,this.mat('#b6a78d'),root,[0,.07,0]);
    if(o.roofType==='gable') {
      const v=[-w/2,eave,-d/2,0,h,-d/2,w/2,eave,-d/2,-w/2,eave,d/2,0,h,d/2,w/2,eave,d/2];
      const geo=new THREE.BufferGeometry();geo.setAttribute('position',new THREE.Float32BufferAttribute(v,3));geo.setIndex([0,1,3,1,4,3,1,2,4,2,5,4,0,2,1,3,4,5]);geo.computeVertexNormals();
      const m=this.mesh(geo,ivory,root);m.userData.part='roof';
    } else if(o.roofType==='flat') { const r=this.boxMesh(w,.2,d,ivory,root,[0,h,0]);r.userData.part='roof'; }
    else this.polygonRoof(w,d,eave,h,o.roofType==='hexagon'?6:4,ivory,root);
    for(const x of [-w/2+.08,w/2-.08]) for(const z of [-d/2+.08,d/2-.08]) this.boxMesh(.13,eave,.13,frame,root,[x,eave/2,z]);
    // Gable marquees have repeated structural bays; roof and wall controls stay independent.
    if(d>12) for(let z=-d/2+5;z<d/2-2;z+=5) for(const x of [-w/2+.08,w/2-.08]) this.boxMesh(.12,eave,.12,frame,root,[x,eave/2,z]);
    const clear = this.mat('#aac2c1',{transparent:true,opacity:.23,metalness:.1,roughness:.18,depthWrite:false,side:THREE.DoubleSide});
    const sideMat=o.metadata?.wallStyle==='solid'?ivory:clear;
    for(const x of [-w/2,w/2]) {const wall=this.boxMesh(.07,eave-.2,d,sideMat,root,[x,eave/2,0]);wall.userData.part='wall';}
    const rear=this.boxMesh(w,eave-.2,.07,sideMat,root,[0,eave/2,-d/2]);rear.userData.part='wall';
    const opening=Math.min(3,w*.6), side=(w-opening)/2;
    for(const x of [-(opening+side)/2,(opening+side)/2]) {const wall=this.boxMesh(side,eave-.2,.07,sideMat,root,[x,eave/2,d/2]);wall.userData.part='wall';}
  }
  label(o) {
    const c=document.createElement('canvas');c.width=512;c.height=100; const ctx=c.getContext('2d');
    ctx.fillStyle='rgba(24,43,41,.9)';ctx.beginPath();ctx.roundRect(1,1,510,98,20);ctx.fill();
    ctx.fillStyle='#f7f4e9';ctx.font='500 31px Arial';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(o.name.length>27?o.name.slice(0,26)+'…':o.name,256,50,475);
    ...
    sprite.position.set(o.position[0],o.position[1]+o.dimensions[1]+2,o.position[2]); sprite.scale.set(16,3.125,1);sprite.userData.label=true;this.labels.add(sprite);
  }
  async model(asset) {
  ...
  addObject(o,generation) {
    ...
    if(o.kind==='tent') this.tent(o,root);
      ...
      const material=this.mat(o.color||(o.kind==='water'?'#3b8f98':'#a5ac83'),{...(o.kind==='water'?{roughness:.22,metalness:.35}:{}),polygonOffset:true,polygonOffsetFactor:-1,polygonOffsetUnits:-1-layer});
      if(o.points?.length>2)this.shape(o.points,material,root,offset);else this.boxMesh(w,Math.max(.02,h),d,material,root,[0,h/2+offset,0]);
      ...
      this.boxMesh(w,h,d,mat,root,[0,h/2,0]);this.boxMesh(w+.2,.18,d+.2,this.mat('#847c6a'),root,[0,h+.1,0]);
    } else if(o.kind==='stage') {
      if(o.points?.length>2){this.shape(o.points,this.mat('#8e887e'),root,h);}
      else this.boxMesh(w,h,d,mat,root,[0,h/2,0]);
      this.boxMesh(w,.12,d,this.mat('#353b38'),root,[0,h+.06,0]);
    } else if(o.kind==='sign') {
  ...
  instances(objects,kind) {
    ...
    const parts=kind==='car'?[{geo:new THREE.BoxGeometry(.84,.45,1),color:'#f0e9d9',p:[0,.28,0]},{geo:new THREE.BoxGeometry(.65,.4,.5),color:'#435a59',p:[0,.62,-.05]},{geo:new THREE.BoxGeometry(1,.22,.12),color:'#242827',p:[0,.14,.33]},{geo:new THREE.BoxGeometry(1,.22,.12),color:'#242827',p:[0,.14,-.33]}]:[{geo:new THREE.CylinderGeometry(.025,.04,.64,5),color:'#8a7860',p:[0,.32,0]},{geo:new THREE.ConeGeometry(.52,.3,7),color:'#657b4e',p:[0,.82,0]},{geo:new THREE.ConeGeometry(.46,.23,7),color:'#7b8d56',p:[0,.68,0]}];
    const dummy=new THREE.Object3D();
    for(const part of parts) {const inst=new THREE.InstancedMesh(part.geo,this.mat(part.color),objects.length);inst.userData.instanceIds=objects.map(o=>o.id);inst.castShadow=true;inst.receiveShadow=true;
      objects.forEach((o,i)=>{const r=new THREE.Matrix4().makeRotationY(o.rotation[1]);const p=new THREE.Vector3(part.p[0]*o.dimensions[0],part.p[1]*o.dimensions[1],part.p[2]*o.dimensions[2]).applyMatrix4(r);dummy.position.fromArray(o.position).add(p);dummy.rotation.set(...o.rotation);dummy.scale.fromArray(o.dimensions);dummy.updateMatrix();inst.setMatrixAt(i,dummy.matrix);});inst.instanceMatrix.needsUpdate=true;inst.computeBoundingSphere();this.content.add(inst);}
  }
  disposeContent() {
  ...
  setScene(data,assets=[]) {
    ...
    this.applyVisibility();if(this.selectedId)this.select(this.selectedId);
    if(!this.initialized){this.initialized=true;const v=data.views?.[0];if(v)this.goTo(v,false);else this.home();}
  }
  applyVisibility() {this.content.traverse(o=>{if(o.userData.part==='roof')o.visible=this.roofs;if(o.userData.part==='wall')o.visible=this.walls;});this.labels.visible=this.showLabels&&this.mode!=='walk';}
  updateLabels() {if(!this.labels.visible)return;const height=this.container.clientHeight||600;for(const label of this.labels.children){const distance=this.camera.position.distanceTo(label.position),worldPerPixel=this.camera.isOrthographicCamera?(this.camera.top-this.camera.bottom)/(height*this.camera.zoom):2*distance*Math.tan(THREE.MathUtils.degToRad(this.camera.fov/2))/height;const width=Math.min(16,130*worldPerPixel);label.scale.set(width,width*100/512,1);}}
  ...
  setLighting(evening) {
    this.shadowDirty=true;
    this.world.background.set(evening?'#293947':'#dce8e7');this.world.fog.color.copy(this.world.background);
    this.ambient.intensity=evening?1.3:2.1;this.sun.intensity=evening?1.7:3.2;this.sun.color.set(evening?'#ffc27e':'#fff1d3');this.sun.position.set(-90,evening?38:150,-65);this.renderer.toneMappingExposure=evening?1.1:1.18;
  }
  select(id) {
  ...
  setMode(mode){if(mode===this.mode)return;const previous=this.camera;this.transition=null;this.mode=mode;this.stopTour();this.camera=mode==='top'?this.orthographic:this.perspective;this.controls.object=this.camera;this.transform.camera=this.camera;
    ...
    if(mode==='top'){const b=this.data?.site.bounds||{minX:-250,maxX:250,minZ:-130,maxZ:130};this.topSpan=Math.max(b.maxX-b.minX,(b.maxZ-b.minZ)*this.container.clientWidth/this.container.clientHeight)*.58;this.camera.position.set((b.minX+b.maxX)/2,600,(b.minZ+b.maxZ)/2+.01);this.camera.up.set(0,0,-1);this.controls.target.set((b.minX+b.maxX)/2,0,(b.minZ+b.maxZ)/2);this.controls.enableRotate=false;this.resize();}
    else{this.controls.enableRotate=true;this.camera.up.copy(UP);if(mode==='walk'){const e=new THREE.Euler().setFromQuaternion(previous.quaternion,'YXZ');this.yaw=e.y;this.pitch=0;const target=this.controls.target;this.camera.position.set(target.x,1.65,target.z+8);}else if(previous===this.orthographic)this.home();}
  ...
  dispose(){this.disposed=true;cancelAnimationFrame(this.frame);this.resizeObserver.disconnect();this.cleanup.forEach(fn=>fn());this.transform.dispose();this.controls.dispose();this.disposeContent();for(const template of this.modelTemplates.values())disposeModel(template);this.modelTemplates.clear();this.modelCache.clear();for(const m of this.materials.values())m.dispose();this.box.geometry.dispose();this.box.material.dispose();this.renderer.dispose();this.renderer.domElement.remove();}
}
