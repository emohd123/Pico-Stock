...
export class EventStudioEngine {
  constructor(container, callbacks = {}) {
    ...
    listen(canvas, 'pointerdown', e => { this.down = { x: e.clientX, y: e.clientY }; canvas.focus();
    ...
    listen(canvas, 'keydown', e => { if (['KeyW','KeyA','KeyS','KeyD','KeyQ','KeyE','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','ShiftLeft','ShiftRight'].includes(e.code)) { this.keys.add(e.code); if (this.mode === 'walk') e.preventDefault(); } });
  ...
  instances(objects,kind) {
    ...
    for(const part of parts) {const inst=new THREE.InstancedMesh(part.geo,this.mat(part.color,{side:part.doubleSide?THREE.DoubleSide:THREE.FrontSide,...(part.tint?{metalness:.3,roughness:.38,envMapIntensity:.45}:{})}),objects.length);inst.userData.instanceIds=objects.map(o=>o.id);inst.castShadow=true;inst.receiveShadow=true;
      objects.forEach((o,i)=>{const r=new THREE.Matrix4().makeRotationY(o.rotation[1]);const p=new THREE.Vector3(part.p[0]*o.dimensions[0],part.p[1]*o.dimensions[1],part.p[2]*o.dimensions[2]).applyMatrix4(r);dummy.position.fromArray(o.position).add(p);dummy.position.y+=terrainHeight(this.data?.site,o.position[0],o.position[2]);dummy.rotation.set(...o.rotation);dummy.scale.fromArray(o.dimensions);dummy.updateMatrix();inst.setMatrixAt(i,dummy.matrix);
        ...
        else if(part.vary){paint.set(o.color||part.color).offsetHSL((idNoise(o.id)-.5)*.045,(idNoise(o.id+'s')-.5)*.14,(idNoise(o.id+'l')-.42)*.12);inst.setColorAt(i,paint);}});
  ...
  buildSkyEnvironment(evening) {
    ...
    const ground = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: evening ? '#2b3226' : '#6f7d4e', side: THREE.BackSide }),
    );
  ...
  setLighting(evening) {
    ...
    this.ambient.intensity=evening?.82:1.02;this.sun.intensity=evening?1.7:3.15;this.sun.color.set(evening?'#ffc27e':'#fff4de');this.setSunDirection(-90,evening?38:150,-65);this.renderer.toneMappingExposure=evening?1.1:1;
  ...
  walkBy(dx,dz){
    ...
    p.y=(this.data?.site.groundY||0)+terrainHeight(this.data?.site,p.x,p.z)+1.65;
  }
  ...
  animate(now){if(this.disposed)return;this.frame=requestAnimationFrame(this.animate);const dt=Math.min(.05,this.clock.getDelta());
    ...
    if(this.mode==='walk'){const held=code=>this.keys.has(code)?1:0;const speed=(held('ShiftLeft')||held('ShiftRight')?12:3.2)*dt;
      ...
      const p=this.camera.position;p.y=(this.data?.site.groundY||0)+terrainHeight(this.data?.site,p.x,p.z)+1.65;
  ...
  dispose(){this.disposed=true;cancelAnimationFrame(this.frame);this.resizeObserver.disconnect();this.cleanup.forEach(fn=>fn());for(const line of this.floorGrid.children){line.geometry?.dispose();line.material?.dispose();}this.floorGrid.clear();this.contactShadowGeometry?.dispose();this.contactShadowMaterial?.map?.dispose();this.sky?.material.dispose();this.transform.dispose();this.controls.dispose();this.disposeContent();for(const template of this.modelTemplates.values())disposeModel(template);this.modelTemplates.clear();this.modelCache.clear();for(const m of this.materials.values())m.dispose();this.environmentTarget?.dispose();this.box.geometry.dispose();this.box.material.dispose();this.renderer.dispose();this.renderer.domElement.remove();}
}
