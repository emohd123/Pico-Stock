...
import { scaledFootprint, localGroundPoint, tentFootprint } from './eventStudioLayout.js';
...
export class EventStudioEngine {
  ...
  label(o) {
    ...
    sprite.position.set(o.position[0],o.position[1]+o.dimensions[1]+2,o.position[2]);
  ...
  addObject(o,generation) {
    ...
    if(o.kind==='tent') this.tent(o,root);
      ...
      const material=o.kind==='path'?this.mat(o.color||'#b9b4a6',{roughness:.92}):this.surfaceMat(o.color||(o.kind==='water'?'#466d70':'#5f853c'),o.kind==='water'?'water':'grass',{...(o.kind==='water'?{roughness:.2,metalness:.28}:{}),polygonOffset:true,polygonOffsetFactor:-1,polygonOffsetUnits:-1-layer});
      const points=scaledFootprint(o);
      if(o.points?.length>2)this.shape(points,material,root,offset);else this.boxMesh(w,Math.max(.02,h),d,material,root,[0,h/2+offset,0]);
  ...
  updateFloorGrid(){
    ...
    lines.position.set(host.position[0],host.position[1]+.155,host.position[2]);lines.rotation.y=host.rotation?.[1]||0;lines.renderOrder=2;
    group.add(lines);
  }
