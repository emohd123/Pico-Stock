...
export async function addProduct(product) {
    if (shouldPreferDirectDb()) {
        ...
        const { rows } = await directQuery(
            `INSERT INTO products (id, name, description, category, price, cost_price, currency, image, stock, in_stock, featured)
...
export async function updateProduct(id, updates) {
    ...
    if (updates.costPrice   !== undefined) rowUpdates.cost_price  = updates.costPrice === null || updates.costPrice === '' ? null : Number(updates.costPrice);
...
function toProductRow(p) {
    return {
        ...
        cost_price:  p.costPrice === null || p.costPrice === undefined || p.costPrice === '' ? null : Number(p.costPrice),
...
/**
 * Returns minimal order rows (status + items only) needed for stock reservation
 * calculation. Uses the order_stock_info view which is readable by the anon role,
 * so this works without a service key or DATABASE_URL.
 */
export async function getOrderStockInfo() {
    if (shouldPreferDirectDb()) {
        const { rows } = await directQuery('SELECT status, items FROM orders');
        return rows.map(r => ({ status: r.status || 'pending', items: r.items || [] }));
    }
    const { data, error } = await supabase
        .from('order_stock_info')
        .select('status, items');
    if (error) throw error;
    return (data || []).map(r => ({ status: r.status || 'pending', items: r.items || [] }));
}
