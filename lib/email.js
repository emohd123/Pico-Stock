...
/**
 * Send a Zoho Books quote PDF to the customer after admin approval.
 */
export async function sendQuoteEmail({ order, pdfBuffer, estimateNumber, toEmail, ccEmail }) {
    const transporter = nodemailer.createTransport({
        host: process.env.SMTP_HOST || 'smtp.gmail.com',
        port: parseInt(process.env.SMTP_PORT || '587'),
        secure: false,
...
<body style="margin:0;padding:20px;background:#f1f5f9;font-family:Arial,sans-serif;">
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,0.1);">
    <div style="background:linear-gradient(135deg,#00A5A5,#007f7f);padding:32px 30px;text-align:center;">
      ...
      <p style="margin:6px 0 0;color:rgba(255,255,255,0.85);font-size:13px;">Quotation — ${dateStr}</p>
    </div>
    <div style="padding:28px 30px;">
      <p style="margin:0 0 16px;font-size:14px;color:#374151;">Dear <strong>${esc(order.exhibitor?.name || 'Exhibitor')}</strong>,</p>
      <p style="margin:0 0 16px;font-size:14px;color:#374151;">
        Thank you for your interest. Please find attached your official quotation
        <strong>${estimateNumber ? `(${estimateNumber})` : ''}</strong> for the following rental:
      </p>
      <div style="background:#f8fafc;border-radius:8px;border:1px solid #e5e7eb;padding:16px;margin-bottom:20px;">
        <table style="width:100%;font-size:13px;border-collapse:collapse;">
          <tr>
            <td style="padding:3px 0;color:#6b7280;width:130px;">Event</td>
            <td style="color:#111827;font-weight:600;">${esc(order.exhibitor?.eventName || '—')}</td>
          </tr>
          <tr>
            <td style="padding:3px 0;color:#6b7280;">Booth</td>
            <td style="color:#111827;font-weight:600;">${esc(order.exhibitor?.boothNumber || '—')}</td>
          </tr>
          <tr>
            <td style="padding:3px 0;color:#6b7280;">Rental Period</td>
            <td style="color:#111827;font-weight:600;">${days} ${days === 1 ? 'day' : 'days'}</td>
          </tr>
          <tr>
            <td style="padding:3px 0;color:#6b7280;">Total</td>
            <td style="color:#00A5A5;font-weight:800;font-size:16px;">${(order.grandTotal || order.total || 0).toFixed(2)} BHD</td>
          </tr>
        </table>
      </div>
    ...
    // Start with Zoho quote PDF
    const quoteAttachments = pdfBuffer ? [{
        filename: `Pico-Quote-${estimateNumber || formatOrderReference(order.id)}.pdf`,
        content: pdfBuffer,
        contentType: 'application/pdf',
    }] : [];
    ...
    const mailOptions = {
        ...
        to: toAddr,
        ...(ccAddr ? { cc: ccAddr } : {}),
        subject: `Pico Stock — Quotation ${estimateNumber || ''} | ${esc(order.exhibitor?.company || order.exhibitor?.name || '')}`,
        html,
