...
/**
 * Notify Pico of a new event marketplace request and copy the client.
 * Prices in `request` were resolved server-side against the event catalogue.
 */
export async function sendEventRequestEmail({ event, request, pdfAttachment = null }) {
    const transporter = nodemailer.createTransport({
        host: process.env.SMTP_HOST || 'smtp.gmail.com',
        port: parseInt(process.env.SMTP_PORT || '587'),
        secure: false,
        ...
        // The client is waiting on this request; never let a slow mail server hold it for minutes.
    ...
    const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'ebrahim@picobahrain.com';
    const currency = esc(request.currency || event?.currency || 'BHD');
    const money = (value) => `${(Number(value) || 0).toFixed(3)} ${currency}`;

    const recipients = new Set([ADMIN_EMAIL]);
    if (event?.notifyEmail) recipients.add(String(event.notifyEmail).trim());
    if (event?.contactEmail) recipients.add(String(event.contactEmail).trim());
    const toAddr = Array.from(recipients).filter(Boolean).join(', ');
    const clientEmail = String(request.contact?.email || '').trim();

    const itemsHtml = (request.items || []).map((item) => `
        <tr style="border-bottom:1px solid #e5e7eb;">
          <td style="padding:10px 12px;">
            <div style="font-weight:600;color:#111827;font-size:13px;">${esc(item.name)}</div>
            ${item.comment ? `<div style="color:#6b7280;font-size:11px;margin-top:2px;">${esc(item.comment)}</div>` : ''}
          </td>
          <td style="padding:10px 12px;text-align:center;font-weight:600;color:#111827;">${Number(item.quantity) || 1}</td>
          <td style="padding:10px 12px;text-align:right;color:#374151;">${item.eventPrice > 0 ? money(item.eventPrice) : 'On request'}</td>
          <td style="padding:10px 12px;text-align:right;font-weight:700;color:#00A5A5;">${item.eventPrice > 0 ? money(item.lineTotal ?? item.eventPrice * item.quantity) : '—'}</td>
        </tr>`).join('');

    const dateStr = new Date(request.createdAt || Date.now()).toLocaleDateString('en-GB', {
        day: '2-digit', month: 'long', year: 'numeric',
    });
...
<!DOCTYPE html>
<html lang="en">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Event Request</title></head>
<body style="margin:0;padding:20px;background:#f1f5f9;font-family:Arial,sans-serif;">
  <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,0.1);">
    <div style="background:linear-gradient(135deg,#00A5A5,#007f7f);padding:28px 30px;text-align:center;">
      <h1 style="margin:0;color:#fff;font-size:20px;font-weight:700;letter-spacing:0.5px;">${esc(event?.name || 'EVENT REQUEST')}</h1>
      <p style="margin:6px 0 0;color:rgba(255,255,255,0.85);font-size:13px;">Request ${esc(request.reference)} — ${dateStr}</p>
    </div>
    <div style="margin:20px 30px 0;padding:16px;background:#f8fafc;border-radius:8px;border:1px solid #e5e7eb;">
      <h3 style="margin:0 0 12px;color:#374151;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;">Client Details</h3>
      <table style="width:100%;border-collapse:collapse;font-size:13px;">
        <tr><td style="padding:3px 0;color:#6b7280;width:130px;">Company</td><td style="padding:3px 0;color:#111827;font-weight:600;">${esc(request.company)}</td></tr>
        <tr><td style="padding:3px 0;color:#6b7280;">Contact</td><td style="padding:3px 0;color:#111827;font-weight:600;">${esc(request.contact?.name) || '—'}</td></tr>
        <tr><td style="padding:3px 0;color:#6b7280;">Email</td><td style="padding:3px 0;color:#111827;font-weight:600;">${esc(request.contact?.email) || '—'}</td></tr>
        <tr><td style="padding:3px 0;color:#6b7280;">Phone</td><td style="padding:3px 0;color:#111827;font-weight:600;">${esc(request.contact?.phone) || '—'}</td></tr>
        <tr><td style="padding:3px 0;color:#6b7280;">Stand / Location</td><td style="padding:3px 0;color:#111827;font-weight:600;">${esc(request.contact?.stand) || '—'}</td></tr>
      </table>
    </div>
    <div style="margin:20px 30px 0;">
      <h3 style="margin:0 0 10px;color:#374151;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;">Requested Items (${Number(event?.days) || 1}-day event)</h3>
      <table style="width:100%;border-collapse:collapse;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
        <thead>
          <tr style="background:#f3f4f6;">
            <th style="padding:10px 12px;text-align:left;font-size:11px;color:#6b7280;font-weight:600;text-transform:uppercase;">Item</th>
            <th style="padding:10px 12px;text-align:center;font-size:11px;color:#6b7280;font-weight:600;text-transform:uppercase;">Qty</th>
            <th style="padding:10px 12px;text-align:right;font-size:11px;color:#6b7280;font-weight:600;text-transform:uppercase;">Event Rate</th>
            <th style="padding:10px 12px;text-align:right;font-size:11px;color:#6b7280;font-weight:600;text-transform:uppercase;">Total</th>
          </tr>
        </thead>
        <tbody>${itemsHtml}</tbody>
      </table>
      <div style="margin-top:12px;padding:14px 16px;background:#f0fdfa;border-radius:8px;border:1px solid #a7f3d0;">
        <table style="width:100%;font-size:13px;border-collapse:collapse;">
          <tr><td style="color:#374151;padding:2px 0;">Subtotal</td><td style="text-align:right;color:#111827;font-weight:600;">${money(request.subtotal)}</td></tr>
          <tr><td style="color:#374151;padding:2px 0;">VAT ${Number(request.vatPercent) || 0}%</td><td style="text-align:right;color:#111827;font-weight:600;">${money(request.vatAmount)}</td></tr>
          <tr><td style="color:#374151;padding:6px 0 0;font-size:14px;font-weight:700;">Total to pay</td><td style="text-align:right;padding:6px 0 0;font-size:18px;font-weight:800;color:#00A5A5;">${money(request.total)}</td></tr>
        </table>
      </div>
    </div>
    ${request.contact?.notes ? `
    <div style="margin:16px 30px 0;padding:14px;background:#fffbeb;border-radius:8px;border:1px solid #fde68a;">
      <h3 style="margin:0 0 6px;color:#374151;font-size:12px;text-transform:uppercase;">Notes from client</h3>
      <p style="margin:0;color:#374151;font-size:13px;white-space:pre-wrap;">${esc(request.contact.notes)}</p>
    </div>` : ''}
    ${event?.paymentTerms ? `
    <div style="margin:16px 30px 0;padding:12px 16px;background:#eff6ff;border-radius:8px;border:1px solid #bfdbfe;">
      <p style="margin:0;color:#1e40af;font-size:12px;white-space:pre-wrap;">${esc(event.paymentTerms)}</p>
    </div>` : ''}
    ${pdfAttachment ? `
    <div style="margin:16px 30px 0;padding:12px 16px;background:#eff6ff;border-radius:8px;border:1px solid #bfdbfe;">
      <p style="margin:0;color:#1e40af;font-size:13px;">📎 The request form (with item pictures) is attached as a PDF.</p>
    </div>` : ''}
    <div style="margin:24px 0 0;padding:20px 30px;background:#f8fafc;border-top:1px solid #e5e7eb;text-align:center;">
      <p style="margin:0 0 4px;font-size:13px;font-weight:700;color:#374151;">Pico International (Bahrain)</p>
      <p style="margin:0;font-size:12px;color:#6b7280;">
        Tel: ${esc(event?.contactPhone || '+973 3635 7377')} &nbsp;|&nbsp;
        <a href="mailto:${esc(event?.contactEmail || 'info@picobahrain.com')}" style="color:#00A5A5;">${esc(event?.contactEmail || 'info@picobahrain.com')}</a>
      </p>
    </div>
  </div>
</body>
    ...
    try {
        const result = await transporter.sendMail({
            from: `"Pico Exhibition Services" <${process.env.EMAIL_FROM || ADMIN_EMAIL}>`,
            to: toAddr,
            ...(clientEmail ? { cc: clientEmail, replyTo: clientEmail } : {}),
            subject: `${event?.name || 'Event'} — Request ${request.reference} | ${request.company}`,
            html,
            attachments: pdfAttachment ? [pdfAttachment] : [],
        });
        return { success: true, messageId: result.messageId };
    } catch (error) {
        console.error('Event request email failed:', error);
        return { success: false, error: error.message };
    }
}
