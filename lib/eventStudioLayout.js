...
/**
 * Height of the measured ground at a point on the plan, relative to the middle of the site.
 * The site is a plane falling towards the lake end; see site.terrain for how it was measured
 * and for what it deliberately does not claim.
 */
export function terrainHeight(site, x, z) {
  const terrain = site?.terrain;
  if (!terrain || terrain.model !== 'plane') return 0;
  return (terrain.northFallPerMetre || 0) * (x - (terrain.referenceX || 0)) + (terrain.crossFallPerMetre || 0) * z;
}

/** The ground a piece stands on: its own, or the level floor of the tent holding it. */
export function standingHeight(scene, object) {
  const host = object.metadata?.parentTentId && scene?.objects?.find(o => o.id === object.metadata.parentTentId);
  const base = host || object;
  return terrainHeight(scene?.site, base.position[0], base.position[2]);
}
...
/**
 * Only the surfaces that change what it costs to build here. The whole event stands on the
 * fairway and the practice ground, so reporting those would bury the cases that matter.
 */
const COURSE_LABELS = { bunker: 'a bunker', green: 'a putting green', tee: 'a teeing ground', cartpath: 'a cart path' };

/**
 * Existing course surfaces a structure stands on. A marquee pitched over a bunker needs the
 * hollow filled or decked, so it is worth knowing before the build, not on the day.
 */
export function courseFeaturesUnder(scene, object) {
  if (!scene?.objects || !object || !['tent', 'stage', 'building'].includes(object.kind)) return [];
  if (object.metadata?.surface) return []; // a course feature is not standing on itself
  const [width, , depth] = object.dimensions;
  const corners = rectCorners(object.position[0], object.position[2], width, depth, object.rotation?.[1] || 0);
  corners.push([object.position[0], object.position[2]]);
  const found = new Set();
  for (const feature of scene.objects) {
    const surface = feature.metadata?.surface;
    if (!surface || !COURSE_LABELS[surface] || !feature.points?.length) continue;
    const outline = scaledFootprint(feature);
    if (corners.some(([x, z]) => polygonContains(outline, ...localGroundPoint(feature, x, z)))) found.add(surface);
  }
  return [...found].map(surface => COURSE_LABELS[surface]);
}
