...
// On Vercel the filesystem is read-only — never fall back to JSON file writes.
const IS_VERCEL_READONLY = process.env.VERCEL === '1' && !POSTGRES_ENABLED;
let pgPool = null;
...
async function readJsonStore() {
    if (IS_VERCEL_READONLY) return buildBlankState();
...
async function writeJsonStore(state) {
    if (IS_VERCEL_READONLY) return; // read-only filesystem on Vercel
    await ensureJsonStore();
...
function buildEmptyStats(meta = {}) {
    return summarizeStats([], {
        ...buildBlankState().meta,
        ...meta,
    });
}
...
export async function getQuotationAiLibraryStats() {
    try {
        ...
        console.error('[quotation ai library] stats fallback:', error);
