...
function scoreLibraryRecord(queryTokens = [], queryText = '', record = {}) {
    ...
    if (record.customer_name && query.includes(String(record.customer_name).toLowerCase())) score += 14;
    if (record.title && query.includes(String(record.title).toLowerCase())) score += 16;
    if (record.ref && query.includes(String(record.ref).toLowerCase())) score += 12;
...
export async function findRelevantQuotationLibraryRecords({ quotation = {}, brief = '', kind = 'draft', limit = 5 } = {}) {
    ...
    const scored = records
        .map((record) => ({
            ...record,
            lexical_score: scoreLibraryRecord(queryTokens, queryText, record),
        }))
