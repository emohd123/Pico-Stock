...
function detectCustomerNameFromQuotationLines(lines = []) {
    ...
    const candidates = (eventIndex >= 0 ? lines.slice(0, eventIndex) : lines)
        ...
        .filter((line) => !/^applied science university$/i.test(line))
...
async function callClaudeJsonChat({ system, prompt, userText, imageInputs = [], temperature = 0.2 }) {
    const client = getAnthropicClient();
    if (!client) return null;

    const userContent = [{ type: 'text', text: `${prompt}\n\n${userText}` }];
    for (const image of imageInputs) {
        const imgMatch = String(image.data || '').match(/^data:([^;]+);base64,(.+)$/s);
        if (!imgMatch) continue;
        const mediaType = imgMatch[1];
        if (!['image/jpeg', 'image/png', 'image/gif', 'image/webp'].includes(mediaType)) continue;
        userContent.push({
            type: 'image',
            source: { type: 'base64', media_type: mediaType, data: imgMatch[2] },
        });
    }

    const response = await client.messages.create({
        model: normalizeText(process.env.ANTHROPIC_MODEL) || 'claude-opus-4-5',
        max_tokens: 8192,
        temperature,
        system,
    ...
    return safeJsonParse(jsonBlock ? jsonBlock[1] : text, null);
}

async function callJsonChat({ system, prompt, userText, imageInputs = [], temperature = 0.2 }) {
    // Try Claude first — better vision and instruction following for structured JSON
    const claudeResult = await callClaudeJsonChat({ system, prompt, userText, imageInputs, temperature });
...
async function extractFileContext(file = {}) {
    ...
    // Large files uploaded via Vercel Blob: fetch buffer from URL
    let blobBuffer = null;
    if (!file.data && file.url) {
        try {
            const res = await fetch(file.url);
            if (res.ok) blobBuffer = Buffer.from(await res.arrayBuffer());
        } catch {
            // ignore fetch errors; buffer stays null
        }
    }

    const decoded = file.data ? decodeDataUrl(file.data) : null;
    const fallbackPath = !decoded && !blobBuffer && file.path ? await readAttachmentFromPath(file.path) : null;
    const mimeType = decoded?.mimeType || type || fallbackPath?.type || '';
    const buffer = blobBuffer || decoded?.buffer || fallbackPath?.buffer || null;
    ...
    try {
        ...
        if (mimeType.startsWith('image/')) {
            // For blob-uploaded images, reconstruct a data URL from the buffer
            const imageData = file.data || (blobBuffer ? `data:${mimeType};base64,${blobBuffer.toString('base64')}` : null);
            return {
                name,
                type: mimeType,
                text: '',
                image: imageData ? { name, data: imageData } : null,
            };
        }
    } catch (error) {
...
- If images are provided (design renders, booth visuals, floor plans, stand drawings):
  • Study the image thoroughly before listing any items.
  • Walk through the design systematically: structure/frame → walls/panels → flooring → ceiling/fascia → lighting → AV/screens → furniture/counters → graphics/signage → special features.
  • For EACH visible element, create a separate line item with a clear description, realistic qty, and correct unit (sqm for area, nos/sets for countable items, rm for linear metres, l.s. for lump-sum).
  • Estimate sizes from context clues (adjacent elements, booth overall footprint) — note your assumptions.
  • Do NOT group unrelated elements into a single item. Separate every distinct element.
  • Identify materials where visible (fabric, glass, wood-look, metal, carpet, vinyl).
...
- Write the project_title and subject to be clear and professional (e.g. "Supply and Installation of Exhibition Stand – BIHE 2025").
...
- Output ONLY valid JSON. No markdown, no explanation outside the JSON.`,
    ...
    return {
        provider: normalizeText(process.env.ANTHROPIC_API_KEY) ? 'claude' : 'openai',
