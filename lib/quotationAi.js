...
import { findRelevantQuotationLibraryRecords, getQuotationAiLibraryStats } from '@/lib/quotationAiLibrary';
...
function buildFallbackDraft({ quotation = {}, brief = '', extractedTexts = [], matchedRecords = [] }) {
    ...
    return {
        ...
        draft_patch: {
            project_title: title,
            subject: title,
            exclusions: quotation.exclusions?.length ? quotation.exclusions : (matchedRecords[0]?.exclusions?.length ? matchedRecords[0].exclusions : defaults.exclusions),
            terms: quotation.terms?.length ? quotation.terms : (matchedRecords[0]?.terms?.length ? matchedRecords[0].terms : defaults.terms),
            payment_terms: quotation.payment_terms?.length ? quotation.payment_terms : (matchedRecords[0]?.payment_terms?.length ? matchedRecords[0].payment_terms : defaults.payment_terms),
            notes: normalizeText(quotation.notes),
            sections: items.length
                ? (matchedSections.length
                    ? matchedSections.slice(0, 3).map((section, index) => ({
                        name: section.name || (index === 0 ? 'Scope of Works' : `Section ${index + 1}`),
                        selling_rule: quotation.sections?.[0]?.selling_rule || section.selling_rule || '0.70',
                        section_selling: 0,
                        items: index === 0 ? items : [],
                    }))
                    : [
                        {
                            name: 'Scope of Works',
                            selling_rule: quotation.sections?.[0]?.selling_rule || '0.70',
                            section_selling: 0,
                            items,
                        },
                    ])
    ...
    (quotation.sections || []).forEach((section, sectionIndex) => {
        (section.items || []).forEach((item, itemIndex) => {
            ...
            const scoredHistorical = matchedItems
                .map((candidate) => ({ candidate, score: scoreCandidateAgainstDescription(item.description, candidate) }))
                .filter((entry) => entry.score > 0)
                .sort((left, right) => right.score - left.score)
                .slice(0, 1);
            ...
            if (scored.length) {
                ...
                suggestions.push({
                    ...
                    confidence: Math.min(0.6 + (score * 0.08), 0.94),
            ...
            suggestions.push({
                ...
                confidence: Math.min(0.48 + (score * 0.07), 0.82),
...
function scoreCandidateAgainstDescription(description, candidate = {}) {
    const descriptionTokens = new Set(keywordTokens(description));
    if (!descriptionTokens.size) return 0;
    ...
    return candidateTokens.reduce((score, token) => score + (descriptionTokens.has(token) ? 1 : 0), 0);
}
