...
async function classifyFile({ pathCtx, fnCtx, extraction, typeInfo, fileInfo }) {
    const reasons = [];
    ...
    const client = pathCtx.client || (fnCtx.clientConfidence >= 0.08 ? normalizeClientLabel(fnCtx.likelyClient) : null) || null;
    const project = pathCtx.project || (fnCtx.projectConfidence >= 0.1 ? normalizeProjectLabel(fnCtx.likelyProject) : null) || null;
    const campaign = pathCtx.campaign || null;
    const department = pathCtx.department || null;
    const documentType = chooseDocumentType(pathCtx.documentType, fnCtx.likelyType, contentSignals.documentType, typeInfo.category);
