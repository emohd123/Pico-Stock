...
function mixHex(hex, targetHex, amount) {
    ...
    return rgbToHex({ r: s.r + (t.r - s.r) * ratio, g: s.g + (t.g - s.g) * ratio, b: s.b + (t.b - s.b) * ratio });
}
...
function parseStandSize(standSize = '') {
    const raw = normalizeText(standSize).toLowerCase();
    const matches = raw.match(/(\d+(?:\.\d+)?)/g) || [];
    let width = Number(matches[0]) || 6;
    let depth = Number(matches[1]) || width;
    // Convert feet to meters when the unit hint is present
    if (/\bft\b|feet|'/.test(raw)) {
        width = Math.round(width * 0.3048 * 10) / 10;
        depth = Math.round(depth * 0.3048 * 10) / 10;
    }
    return {
...
export function normalizeStandDesignScene(raw = {}, brief = {}) {
    ...
    const scene = {
        ...
        objects: Array.isArray(raw.objects)
            ? raw.objects.reduce((acc, item, index) => {
                try {
                    acc.push(normalizeStandDesignSceneObject(item, index));
                } catch (err) {
                    console.warn(`[standDesign] Skipped object[${index}] (${item?.type || '?'}): ${err.message}`);
                }
                return acc;
            }, [])
            : [],
