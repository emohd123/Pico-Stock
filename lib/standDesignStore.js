...
const PRODUCTION_READONLY_FALLBACK = process.env.VERCEL === '1' && !POSTGRES_ENABLED;
...
function ensureStandDesignWritableStorage() {
    if (PRODUCTION_READONLY_FALLBACK) {
        throw new StandDesignStoreError('Production storage is not configured for Stand Design. Set DATABASE_URL in Vercel to enable saved records and generation.', 503);
    }
}
...
export async function getStandDesigns() {
    if (POSTGRES_ENABLED) {
        ...
        return result.rows.map(safeRowToRecord).filter(Boolean);
    }

    const records = await readJsonStore();
    return sortByUpdated(records.map(safeNormalizeRecord).filter(Boolean));
}
...
export function getStandDesignStorageStatus() {
    return {
        mode: POSTGRES_ENABLED ? 'postgres' : 'json',
        production_ready: POSTGRES_ENABLED || !PRODUCTION_READONLY_FALLBACK,
    };
}
