...
const SUPABASE_TIMEOUT_MS = Number(process.env.STAND_DESIGN_SUPABASE_TIMEOUT_MS || 8000);
// Upserts that carry view image paths/data can be larger; give them extra time
const SUPABASE_UPSERT_TIMEOUT_MS = Number(process.env.STAND_DESIGN_SUPABASE_UPSERT_TIMEOUT_MS || 15000);
...
function shouldUseJsonFallback() {
    // JSON file fallback only works in local development.
    // On Vercel the filesystem is read-only — never fall back to JSON;
    // let the Supabase error propagate so we can diagnose it.
...
async function upsertSupabaseRow(supabase, record) {
    ...
    const { error } = await withSupabaseUpsertTimeout(
        supabase.from('stand_designs').upsert(payload),
        'stand design upsert',
    );
    if (error) {
        console.error('[StandDesignStore] Supabase upsert failed:', error.message, error.code, error.details);
        throw new Error(`Supabase upsert failed: ${error.message}`);
    }
}

// Override withSupabaseTimeout for upserts: use the longer timeout
async function withSupabaseUpsertTimeout(promise, label) {
    let timer = null;
    try {
        return await Promise.race([
            promise,
            new Promise((_, reject) => {
                timer = setTimeout(() => {
                    reject(new Error(`Supabase ${label} timed out after ${SUPABASE_UPSERT_TIMEOUT_MS}ms`));
