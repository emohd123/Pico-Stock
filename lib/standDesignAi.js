...
export function getStandDesignAiStatus() {
    return {
        ...
        scene_model: getStandDesignSceneModel(),
    };
}
...
export function getStandDesignSceneModel() {
    return normalizeText(process.env.GEMINI_SCENE_MODEL || 'gemini-3.1-pro-preview');
}
...
function buildSceneGenerationPrompt({ brief = {}, conceptTitle = '', conceptSummary = '', conceptIndex = 0 }) {
    const normalizedBrief = normalizeStandDesignBrief(brief);
    const primitivePalette = getStandDesignPrimitivePalette()
        .map((item) => `${item.type} (${item.label})`)
        .join(', ');
    const assetPalette = getStandDesignAssetPalette()
        .map((item) => `${item.key} => ${item.type} (${item.label})`)
        .join(', ');
    ...
    return [
        ...
        `Allowed primitive types: ${primitivePalette}.`,
        `Allowed asset keys: ${assetPalette}.`,
        `Allowed material presets: ${materialPalette}.`,
        'Rules:',
        ...
        '- Use kind "primitive" only for structural pieces like floor, wall, partition, counter, ramp, plinth, fascia, screen.',
        '- Use kind "asset" only with a valid asset_key from the allowed asset palette.',
...
async function persistSceneRenderImage({ designId, conceptId, imageDataUrl, label = '' }) {
    ...
    await fs.mkdir(GENERATED_DIR, { recursive: true });
    const extension = mimeType === 'image/jpeg' ? 'jpg' : mimeType === 'image/webp' ? 'webp' : 'png';
    const filename = `${designId}-${conceptId}-scene-${Date.now()}.${extension}`;
    const absolutePath = path.join(GENERATED_DIR, filename);
    await fs.writeFile(absolutePath, Buffer.from(imageBytes, 'base64'));
...
export async function generateStandDesignScene({
    ...
    try {
        ...
        const userParts = [
            {
                text: buildReferenceMatchedScenePrompt({
                    brief: normalizedBrief,
                    conceptTitle,
                    conceptSummary,
                    conceptIndex,
        ...
        if (referenceObservations?.match_camera && !rawScene.match_camera) {
            rawScene.match_camera = referenceObservations.match_camera;
        }
        const scene = validateStandDesignScene(rawScene, normalizedBrief);
        const scoring = scoreSceneAgainstReference(scene, referenceObservations, normalizedBrief, conceptViews);
