...
export async function generateStandDesignConceptViews({
    ...
    if (isGeminiPreviewImageModel(configuredModel)) {
        ...
        // Generate all 4 views in parallel to stay within Vercel's function timeout
        const viewResults = await Promise.all(viewsConfig.map(async (view) => {
            const userParts = [{
            ...
            // Use Flash only (no Pro fallback) to avoid cascading 45s timeouts when running in parallel
