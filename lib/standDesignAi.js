...
async function ensureStandDesignStorageBucket() {
    ...
    if (!storageBucketReadyPromise) {
        storageBucketReadyPromise = (async () => {
            ...
            const { data } = await supabase.storage.getBucket(SUPABASE_STORAGE_BUCKET);
            if (data?.id) return true;
            const { error } = await supabase.storage.createBucket(SUPABASE_STORAGE_BUCKET, {
                public: true,
                fileSizeLimit: '20MB',
            });
            if (error && !/already exists/i.test(normalizeText(error.message))) {
...
async function uploadGeneratedAssetToStorage({ storagePath, mimeType, imageBytes, cacheControl = '3600' }) {
    await ensureStandDesignStorageBucket();
    const supabase = getSupabaseStorageClient();
