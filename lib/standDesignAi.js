...
// Views are generated in parallel (Promise.all), so total time = slowest single call.
// 55 s gives Flash enough runway while staying inside Vercel's 60 s function cap.
const GEMINI_PREVIEW_TIMEOUT_MS = Number(process.env.STAND_DESIGN_GEMINI_TIMEOUT_MS || 55000);
...
function createInlineGeneratedAsset({ id = '', label = '', mimeType = 'image/png', imageBytes = '' } = {}) {
    return {
        id,
        label,
        path: toInlineDataUrl(mimeType, imageBytes),
        mimeType,
...
function getConfiguredStandDesignModel() {
...
export function getStandDesignModel() {
    const configured = getConfiguredStandDesignModel();
    if (/^gemini-3-pro-image-preview$/i.test(configured)) {
...
export function getStandDesignFallbackModel() {
    ...
    const rawPrimary = getConfiguredStandDesignModel();
    if (/^gemini-3-pro-image-preview$/i.test(rawPrimary)) {
        return rawPrimary;
    }
    return '';
}
...
function getStandDesignModelCandidates() {
    ...
    const preferred = /^gemini-3-pro-image-preview$/i.test(primary) && fallback
        ? [fallback, primary]
        : [primary, fallback];
    return preferred.filter(Boolean).filter((value, index, array) => array.indexOf(value) === index);
}
        ...
        if (storageMode) {
            try {
                ...
                if (shouldFallbackToLocalGeneratedAssets()) {
                    ...
                    view.path = toInlineDataUrl(mimeType, imageBytes);
                }
            }
        } else if (!inlineMode) {
            view.path = await persistAssetToLocalFile({ designId, conceptId, suffix: angle, mimeType, imageBytes });
...
async function persistSceneRenderImage({ designId, conceptId, imageDataUrl, label = '' }) {
    ...
    if (SUPABASE_STORAGE_ENABLED) {
        try {
            ...
            return createInlineGeneratedAsset({
                id: `${conceptId}-render-${Date.now()}`,
                label: normalizeText(label) || 'Scene Render',
                mimeType,
                imageBytes,
...
export async function generateStandDesignConceptViews({
    ...
    let resolvedModel = configuredModel;
    if (isGeminiPreviewImageModel(configuredModel)) {
        // Load shared assets once before parallelising
        const referenceImage = await loadReferenceImage(referenceImagePath);
        const brandLogoPart = await loadOptionalInlinePart(normalizedBrief.logo_image_path);
        const brandReferencePart = await loadOptionalInlinePart(normalizedBrief.brand_reference_image_path);

        // Generate views in 2 batches of 2 to avoid Gemini rate-limit queueing.
        // 4 simultaneous calls cause the API to queue some requests silently,
        // pushing their latency past the timeout. 2+2 batching keeps load low
        // (~20s per batch, ~40s total) while still being faster than sequential.
        const buildViewParts = (view) => {
            const parts = [{
            ...
            parts.push(createPartFromBase64(referenceImage.imageBytes, referenceImage.mimeType));
            if (brandLogoPart) parts.push(brandLogoPart);
            if (brandReferencePart) parts.push(brandReferencePart);
            return parts;
        };

        const runView = async (view) => {
            const userParts = buildViewParts(view);
            const image = await generateGeminiPreviewImage({ ai, model: configuredModel, userParts });
            if (!image) {
                throw new Error(`Gemini did not return the ${view.label.toLowerCase()} image.`);
            }
            return { image, model: configuredModel };
        };

        // Batch 1: front + back (~20s)
        const batch1 = await Promise.all(viewsConfig.slice(0, 2).map(runView));
        // Batch 2: side + perspective (~20s)
        const batch2 = await Promise.all(viewsConfig.slice(2).map(runView));
        const viewResults = [...batch1, ...batch2];
