...
const GEMINI_PREVIEW_TIMEOUT_MS = Number(process.env.STAND_DESIGN_GEMINI_TIMEOUT_MS || 110000);
...
export async function generateStandDesignConcepts({
    ...
    if (isGeminiPreviewImageModel(configuredModel)) {
        const concepts = await Promise.all(normalizedIndexes.map(async (index) => {
