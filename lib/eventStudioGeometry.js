...
export function carParts() {
  const [uw, uh, ul] = [1 / REFERENCE_CAR[0], 1 / REFERENCE_CAR[1], 1 / REFERENCE_CAR[2]];
  const tyreRadius = 0.33, tyreWidth = 0.22, axle = 0.34, front = 1.55, rear = -1.5;
  const wheels = [];
  for (const x of [-0.86, 0.86]) for (const z of [rear, front]) {
    const wheel = new THREE.CylinderGeometry(1, 1, 1, 16);
    wheel.rotateZ(Math.PI / 2);
    wheel.scale(tyreWidth * uw, tyreRadius * uh, tyreRadius * ul);
    wheel.translate(x * uw, 0, z * ul);
    wheels.push(wheel);
  }
  const box = (w, h, l, y, z = 0) => {
    const geometry = new THREE.BoxGeometry(w * uw, h * uh, l * ul);
    geometry.translate(0, y * uh - axle * uh, z * ul);
    return geometry;
  };
  const taper = (bw, bl, tw, tl, height, y, z, shift) => {
    const geometry = taperedBox(bw * uw, bl * ul, tw * uw, tl * ul, height * uh, shift * ul);
    geometry.translate(0, y * uh - axle * uh, z * ul);
    return geometry;
  };
  return [
    { geo: mergeParts(wheels), color: '#15181a', p: [0, axle * uh, 0] },
    { geo: box(2.05, 0.46, 4.60, 0.53), color: '#ffffff', p: [0, axle * uh, 0], tint: true },
    { geo: taper(2.16, 4.40, 1.98, 3.60, 0.22, 0.87, 0, -0.05), color: '#ffffff', p: [0, axle * uh, 0], tint: true },
    { geo: taper(1.86, 2.55, 1.44, 1.70, 0.44, 1.19, -0.12, -0.12), color: '#26343a', p: [0, axle * uh, 0] },
    { geo: box(2.02, 0.10, 0.34, 0.62, 2.28), color: '#c9ccc7', p: [0, axle * uh, 0] },
    { geo: box(2.02, 0.10, 0.34, 0.62, -2.26), color: '#c9ccc7', p: [0, axle * uh, 0] },
  ];
}
...
export function palmParts() {
  ...
  return [{geo:new THREE.CylinderGeometry(.024,.037,.8,9,5),color:'#766047',p:[0,.4,0],vary:true}, {geo:leaves,color:'#4d703e',p:[0,0,0],doubleSide:true,vary:true}];
}
