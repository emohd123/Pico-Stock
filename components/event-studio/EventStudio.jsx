...
import { tentFootprint, localGroundPoint, worldGroundPoint, freeFloorSlot, occupantsOf, clampIntoFootprint, hostTentAt, courseFeaturesUnder, polygonContains } from '@/lib/eventStudioLayout';
...
export default function EventStudio() {
  ...
  const standsOn=useMemo(()=>courseFeaturesUnder(scene,scene?.objects.find(o=>o.id===selected)),[scene,selected]);
  const onCourse=useMemo(()=>scene?scene.objects.filter(o=>courseFeaturesUnder(scene,o).length).length:0,[scene]);
  ...
  const updateObject=useCallback((id,patch)=>{
    ...
    commitRef.current(d=>{
      ...
      const reseat=piece=>{
        let host=d.objects.find(o=>o.id===piece.metadata?.parentTentId);if(!host||host.id===piece.id)return;
        // A piece whose centre has left its tent belongs where it now stands. Without this it is
        // welded to whichever tent it was first placed in and can never be moved to another.
        const centre=localGroundPoint(host,piece.position[0],piece.position[2]);
        if(!polygonContains(tentFootprint(host),centre[0],centre[1])){
          const moved=hostTentAt(d.objects,piece.position[0],piece.position[2]);
          if(!moved){delete piece.metadata.parentTentId;piece.position[1]=0;return;}
          if(moved.id!==host.id){host=moved;piece.metadata.parentTentId=host.id;piece.zoneId=host.zoneId||piece.zoneId;piece.position[1]=TENT_FLOOR;}
        }
  ...
  const navigate=(next)=>{engine.current?.setMode(next);if(next==='walk'){setEditing(false);announce('Scroll to move through the site, double-click a spot to walk to it, and click once to look around with the mouse. W walks, A and D turn, Q and E step sideways, Shift hurries.');}};
  ...
  const addFurniture=(asset,replace=false)=>{
    ...
    // A replacement inherits the old centre, so a larger piece has to be fitted to the tent again.
    if(replace&&host&&previous){
      const [lx,lz]=localGroundPoint(host,item.position[0],item.position[2]);
      const [cx,cz]=clampIntoFootprint(tentFootprint(host),lx,lz,item.dimensions[0],item.dimensions[2],(item.rotation?.[1]||0)-(host.rotation?.[1]||0));
      const [wx,wz]=worldGroundPoint(host,cx,cz);
      item.position=[+wx.toFixed(3),item.position[1],+wz.toFixed(3)];
    }
  ...
  return <main ref={root} className={`event-studio${full?' es-fullscreen':''}${rail?'':' es-rail-hidden'}${inspector?'':' es-inspector-hidden'}`}>
    ...
    <section className="es-workspace">{recording!==null&&<div className="es-recording-overlay" role="status"><div><i/><strong>Recording walkthrough</strong><span>{recording} / 90 seconds · 1080p</span><small>Keep this tab visible while the tour records.</small><button onClick={()=>recordingRef.current?.cancel()}>Cancel recording</button></div></div>}
      ...
      <div className="es-canvas-area"><div className="es-view-toolbar"><div><Tool label="Show or hide library" icon="layers" active={rail} onClick={()=>setRail(!rail)}/><div className="es-segment">{[['orbit','orbit','Orbit'],['top','map','Plan'],['walk','walk','Walk']].map(([id,icon,label])=><Tool key={id} label={`${label} view`} icon={icon} active={mode===id} onClick={()=>navigate(id)}>{label}</Tool>)}</div></div><div><Tool label="Edit layout" icon="edit" active={editing} onClick={()=>{setEditing(!editing);if(mode==='walk')navigate('orbit');}}/><Tool label="Undo" icon="undo" disabled={!undoStack.length} onClick={undo}/><Tool label="Redo" icon="redo" disabled={!redoStack.length} onClick={redo}/><Tool label="Fullscreen" icon="expand" onClick={()=>document.fullscreenElement?document.exitFullscreen():root.current?.requestFullscreen?.()}/></div></div>
        ...
        {mode==='walk'&&<div className="es-walk-controls"><p>Click to look around · scroll to move · double-click a spot to walk there · <b>W</b> forward · <b>A</b> <b>D</b> turn · <b>Q</b> <b>E</b> sidestep · Shift hurries</p><div>{[['↑',0,-1,'Walk forward'],['↺',-1,0,'Turn left'],['↓',0,1,'Walk back'],['↻',1,0,'Turn right']].map(([label,x,y,title])=><button key={label} aria-label={title} title={title} onPointerDown={e=>{try{e.currentTarget.setPointerCapture(e.pointerId);}catch{}if(engine.current)engine.current.joystick=[x,y];}} onPointerUp={()=>{if(engine.current)engine.current.joystick=[0,0];}} onPointerCancel={()=>{if(engine.current)engine.current.joystick=[0,0];}}>{label}</button>)}</div></div>}
      </div>
