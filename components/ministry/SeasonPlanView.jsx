...
import VenuePicker from '@/components/ministry/VenuePicker';
import PlanRefresh from '@/components/ministry/PlanRefresh';
import PlanShare from '@/components/ministry/PlanShare';
...
/**
 * The season plan. Rendered twice: on the admin page with the venue pickers
 * and the share control, and behind the read-only link handed to management,
 * where the same numbers appear without any way to change them.
 */
export default async function SeasonPlanView({ readOnly = false, shareUrl = null, generatedAt }) {
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#f8fafc', color: '#4D4D4F' }}>
            ...
            <header className="no-print" style={{ borderBottom: '4px solid #00C7B1', background: '#fff' }}>
                ...
                <div style={{ maxWidth: 1500, margin: '0 auto', padding: '0 20px 12px', display: 'flex', alignItems: 'flex-end', gap: 14, flexWrap: 'wrap' }}>
                    <div>
                    ...
                    <div className="no-print" style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                        <PlanRefresh generatedAt={generatedAt} />
                        {shareUrl ? <PlanShare url={shareUrl} /> : null}
                    </div>
                </div>
            </header>
            ...
            <main style={{ maxWidth: 1500, margin: '0 auto', padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                {/* ---- where each meeting is held: the single edit that resolves clashes ---- */}
                <section className="p-card" style={{ ...card, overflow: 'hidden' }}>
                    <h2 style={{ fontSize: 13, fontWeight: 700, margin: 0, padding: '11px 16px', borderBottom: '1px solid #f1f5f9', color: '#22282B' }}>
                        ...
                        <span style={{ marginLeft: 8, fontWeight: 400, fontSize: 11, color: '#94a3b8' }}>
                            {readOnly
                                ? 'a meeting with no venue set has to be planned as its own location'
                                : 'set a venue and the whole plan recalculates — clashes, kept builds and inventory'}
                        </span>
                    </h2>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(330px, 1fr))', gap: 0 }}>
                        {rows.slice().sort((a, b) => {
                            const au = a.venue === VENUE_UNKNOWN ? 0 : 1, bu = b.venue === VENUE_UNKNOWN ? 0 : 1;
                            return au !== bu ? au - bu : a.eventDays[0].localeCompare(b.eventDays[0]);
                        }).map((m) => (
                            <div key={m.key} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 16px', borderTop: '1px solid #f8fafc', background: m.venue === VENUE_UNKNOWN ? '#fef2f2' : 'transparent' }}>
                                ...
                                {readOnly
                                    ...
                                    : <VenuePicker ministryId={m.ministryId} quotationIds={m.quoteIds} venue={m.venue}
                                        unknown={m.venue === VENUE_UNKNOWN} options={venueOptions} />}
                            </div>
                        ))}
                    </div>
                </section>
