...
// meetings run that day, the items each needs, titles, plates, LPO state and
// how many separate builds the day demands. Event days only — setup and
// removal are not part of this plan.
//
                    ...
                    <section key={mo.label} style={{ background: '#fff', borderRadius: 12, boxShadow: '0 1px 2px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)' }}>
                            {mo.cells.map((iso, i) => {
                                ...
                                const hasEvent = Boolean(d && d.entries.length);
                                ...
                                return (
                                    <button key={iso} type="button" onClick={() => setSelected(iso)}
                                        ...
                                        {d ? d.entries.slice(0, 3).map((e, j) => (
                                            <span key={j} title={meetings[e.k].ministry} style={{
                                                display: 'block', height: 6, borderRadius: 2, background: meetings[e.k].color,
                                            }} />
                                        )) : null}
                ...
                {!sel || !selEntries.length ? (
                    ...
                    <>
                        ...
                        {selEntries.map((e) => {
                            ...
                            return (
                                <section key={e.k} style={{ background: '#fff', borderRadius: 12, boxShadow: '0 1px 2px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
                                    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, padding: '10px 14px', borderBottom: '1px solid #f1f5f9' }}>
                                        <span style={{ borderRadius: 4, background: '#f0fdfa', color: '#00857A', padding: '1px 7px', fontSize: 9.5, fontWeight: 700 }}>📍 EVENT</span>
                                        {readOnly
                        ...
                        {dayItems.length && selEntries.length > 1 ? (
