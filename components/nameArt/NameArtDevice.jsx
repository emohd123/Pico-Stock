...
import { NAME_ART_DEFAULTS, cleanGuestName } from '@/lib/nameArt/config';
...
export default function NameArtDevice({ role }) {
  ...
  async function submit(e){e.preventDefault();if(!cleanGuestName(name))return;setBusy(true);setError('');requestId.current ||= crypto.randomUUID();try{const data=await api('/jobs',{method:'POST',body:JSON.stringify({name,accepted,requestId:requestId.current})});setJob(data);requestId.current=null;}catch(e){setError(e.message);if(e.message.includes('start a new'))requestId.current=null;}finally{setBusy(false);}}
  ...
  if(role==='screen')return <main className={styles.screen}>
    ...
    {!job&&<div className={styles.invitation}><small>GREETINGS FROM BAHRAIN</small><h1 dir="rtl">اسمك… حكاية جميلة</h1><p>Your name. A moment to keep.</p><span>{config.paused?'We’ll be back shortly.':'Enter your name on the iPad to begin.'}</span></div>}
    {job&&<aside className={styles.screenQr}><img src={`${API}/poster/${job.shareToken}/qr`} alt="Scan to download your personalised poster"/><div><strong>Yours to keep</strong><span>Scan to download<br/><b dir="rtl">امسح الرمز لحفظ بطاقتك</b></span></div></aside>}
