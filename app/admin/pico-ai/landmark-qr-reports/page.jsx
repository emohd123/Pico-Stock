...
export default function LandmarkQrReportsPage() {
  ...
  return <AdminShell activeSection="landmark-qr"><main className="pico-ai qr-report">
    ...
    {data && <>
      ...
      <section className="pai-card qr-library"><div className="qr-library-head"><div><span className="pai-eyebrow">QR LIBRARY</span><h2>Every landmark, one trackable link</h2><p>Download the QR after the landmark and final destination page are approved.</p></div><div className="qr-filters"><label><span>Country</span><select value={country} onChange={(event) => setCountry(event.target.value)}><option>All</option><option>Saudi Arabia</option><option>Bahrain</option></select></label><label><span>Find landmark</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name or reference"/></label></div></div>
        <div className="pai-table"><table><thead><tr><th>Ref.</th><th>Landmark</th><th>Country</th><th>Scans</th><th>Visitors</th><th>Last scan</th><th>QR</th></tr></thead><tbody>{visible.map((item) => <tr key={item.id}><td><strong>{item.reference}</strong></td><td>{item.name}</td><td><span className={`qr-country ${item.country === 'Bahrain' ? 'bahrain' : ''}`}>{item.country}</span></td><td>{number.format(item.scans)}</td><td>{number.format(item.uniqueVisitors)}</td><td>{item.lastScan ? new Date(item.lastScan).toLocaleString() : '—'}</td><td><a className="qr-download" href={`/api/pico-ai/admin/landmark-qr/${item.id}`} download={`${item.reference}-${item.id}-QR.svg`}>Download QR</a></td></tr>)}</tbody></table></div>
      </section>
    </>}
  </main></AdminShell>;
}
